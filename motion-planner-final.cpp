...
int main(int args,char **argv)
{
    ...
    FILE *fp1, *fp2;
    fp1 = fopen("prefix_file_1.txt", "w");
    fp2 = fopen("suffix_file_1.txt","w");
    ...
    for(int i=0;i<prefix_path.size();i++){
        ...
        fprintf(fp1, "%d,%d\n",prefix_path[i].y,prefix_path[i].x );
    }
    ...
    for(int i=0;i<suffix_path.size();i++){
        ...
        fprintf(fp2, "%d,%d\n",suffix_path[i].y,suffix_path[i].x);
    }
